...
export async function startDaemon(opts) {
    ...
    app.post("/a2a/:agent/:capability", async (req, reply) => {
        const { agent, capability } = req.params;
        const entry = opts.agents.get(agent);
        if (!entry)
            return reply.code(404).send({ error: "unknown_agent", agent });
        const hasCap = entry.card.capabilities.some(c => c.id === capability);
        if (!hasCap)
            return reply.code(404).send({ error: "unknown_capability", agent, capability });
