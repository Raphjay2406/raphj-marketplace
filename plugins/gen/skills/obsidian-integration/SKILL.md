...
After 5-10 completed projects, Genorah can recommend design patterns from empirical data. The Knowledge Base accumulates structured project history that enables cross-project pattern discovery through Obsidian's Dataview plugin and graph view.

### Project History Directory

`Knowledge/project-history/` stores one note per completed project. Each note uses Dataview-compatible frontmatter:

```yaml
---
name: "Rosenstein Real Estate"
...
client: rosenstein
date: 2026-03-28
tags: [archetype-neo-corporate, industry-real-estate, score-30]
---
```

After project close, `/gen:export` populates this note with: archetype choice rationale, palette summary, final Anti-Slop score breakdown, tension techniques used, font pairings, and a retrospective section for lessons learned.

### PATTERNS.md

`Knowledge/project-history/PATTERNS.md` is auto-generated from the project history notes after each new project export. It surfaces recurring patterns across all completed projects -- for example, which archetypes correlate with highest scores in a given industry, or which font pairings appear most frequently in SOTD-Ready projects. This file is regenerated, not manually edited.

### Graph Relationship Notes

`Knowledge/graph/` contains five relationship notes that map cross-project patterns. Each is auto-generated by `/gen:export` after project completion.
...
+-- color-mood-map.md        <- palette mood vs client satisfaction
```

**archetype-industry.md** -- Maps archetype-to-industry pairings with outcome data. Enables recommendations like "Neo-Corporate has a 92% approval rate for real estate clients" after enough data points accumulate.

**font-performance.md** -- Tracks display/body font pairings alongside their Anti-Slop typography sub-scores. Surfaces which pairings consistently score 5/5 and in which archetype contexts.

**tension-impact.md** -- Records which creative tension techniques (Scale Violence, Material Collision, etc.) were used per project and how they correlated with Creative Courage scores. Helps agents recommend tension techniques with proven impact.

**integration-patterns.md** -- Tracks which third-party integrations (HubSpot, Stripe, Shopify, etc.) appear together per industry vertical. Helps the researcher agent recommend integration stacks based on empirical frequency.

**color-mood-map.md** -- Maps palette mood descriptors (warm, cool, muted, vibrant) to client satisfaction outcomes. Feeds back into the creative director agent's palette recommendations.

### Dataview Query Examples

**Top-performing archetypes by industry:**

````markdown
```dataview
TABLE archetype, score, industry
FROM "project-history"
WHERE type = "project-history"
SORT score DESC
GROUP BY industry
```
````

**Font pairings with highest typography scores:**

````markdown
```dataview
TABLE display-font, body-font, typography-score, archetype
FROM "graph/font-performance"
SORT typography-score DESC
LIMIT 10
```
````

**Creative tensions that improved scores:**

````markdown
```dataview
TABLE tension-type, score-delta, archetype, project
FROM "graph/tension-impact"
WHERE score-delta > 0
SORT score-delta DESC
```
````

### How /gen:export Populates the Knowledge Graph

After project completion, `/gen:export` performs these steps:

1. Read the project's DESIGN-DNA.md, BRAINSTORM.md, and final Anti-Slop scores
2. Create or update the project note in `Knowledge/project-history/{project-slug}.md`
3. Append archetype-industry pairing data to `graph/archetype-industry.md`
4. Append font pairing and typography score to `graph/font-performance.md`
5. Append tension technique usage and Creative Courage score to `graph/tension-impact.md`
6. Append integration stack to `graph/integration-patterns.md`
7. Append palette mood and client satisfaction to `graph/color-mood-map.md`
8. Regenerate `project-history/PATTERNS.md` from all project history notes

After 5-10 projects, the accumulated data enables Genorah to make empirical recommendations during the research and creative direction phases, replacing heuristic defaults with patterns proven across real projects.
...
2. **Zero isolated nodes** -- Every file must have at least one outgoing `[[wiki-link]]` to another file
3. **Hub-and-spoke** -- `_Dashboard.md` links to every category; each category links to its members; members cross-link to related skills
4. **Filename-only links** -- Use `[[skill-name]]` not `[[path/to/skill-name]]`. Obsidian resolves by filename regardless of folder

### Color Group Standard

Every Obsidian vault generated by Genorah uses these standardized color groups in `.obsidian/graph.json`:

| Group | Path Query | Color (RGB int) | Hex | Visual |
|-------|-----------|-----------------|-----|--------|
| Core Skills | `path:Skills/core` | 5046016 | `#4D0080` | Purple-red |
| Domain Skills | `path:Skills/domain` | 34816 | `#008800` | Green |
| Utility Skills | `path:Skills/utility` | 11141290 | `#AA00AA` | Blue-teal |
| Commands | `path:Commands` | 16744448 | `#FF8800` | Orange |
| Pipeline Agents | `path:Agents/pipeline` | 16711680 | `#FF0000` | Red |
| Specialist Agents | `path:Agents/specialists` | 16711935 | `#FF00FF` | Magenta |
| Protocol Agents | `path:Agents/protocols` | 8388736 | `#800080` | Purple |
| Dashboard | `file:_Dashboard` | 16776960 | `#FFFF00` | Yellow |
...
If the graph has disconnected nodes after a sync:
1. Run the connectivity pass again (convert references → add See Also → verify incoming links)
2. Check if new skills were added without cross-references in their Layer 3
3. Add missing skills to `_Dashboard.md` under the appropriate category
