...
This skill is **always active** during every build execution. It defines the multi-level reporting protocol that the design-lead, build-orchestrator, and section-builder agents follow to keep users informed and maintain machine-readable state.
...
| Section complete | Tier 2 (section) | Compact inline highlight | Conversation | design-lead |
| Section fails | Tier 2 (section) | Inline alert + link | Conversation | design-lead |
| Wave complete | Tier 3 (wave) | Detailed summary block | Conversation + STATE.md | design-lead |
| Build complete | Tier 4 (milestone) | Full report with scores | Conversation + STATE.md | design-lead |
| User requests status | Any | Current STATE.md task table | Conversation | design-lead |
...
- **Referenced by:** `design-lead` agent during wave orchestration -- consumes Tier 2/3/4 templates
...
The design-lead reads each builder's SUMMARY.md and batches the updates into STATE.md.

### Pattern 2: Section Completion Highlight (Tier 2)

When a section completes (all tasks done + SUMMARY.md written), the design-lead reports a **compact one-liner** in conversation. This is the only per-section conversation output.
...
After **all sections** in a wave complete, the design-lead produces a detailed wave summary. This is the user's **review gate** -- builds pause here until the user approves.
...
2. **Always include canary check results.** The canary check is performed by the design-lead before writing the wave summary (see design-lead agent protocol).
3. **Always show next wave preview.** Users need to know what comes next before approving.
4. **Always end with "Awaiting user approval".** This is a HARD gate. The design-lead does NOT proceed to the next wave until the user explicitly approves.
...
Run `/modulo:verify` for full visual QA review.
...
- Triggered only by explicit user request (e.g., "show me what the hero looks like") or via `/modulo:verify`
...
- Wave summary notes: "Screenshots unavailable (no browser tools). Run `/modulo:verify` with browser tools for visual QA."
...
STATE.md has a **100-line budget** (established in the design-lead agent protocol). Progress reporting must stay within this budget by using compact formats and scoped data.
...
Review gates pause execution after every wave for user approval. This is a HARD requirement -- the design-lead never auto-proceeds.
...
Wave N sections all COMPLETE
    |
    v
Design-lead produces wave summary (Tier 3)
    |
    v
...
USER DECISION (one of):
    ...
    |
    v
Once approved: design-lead spawns Wave N+1 builders
```
...
- Design-lead canary check -- 5-question memory fidelity score
...
**Instead:** Per-task updates go to STATE.md only (machine-readable table rows). Conversation gets per-section one-liners (Tier 2). Users who want task-level detail can ask for current STATUS and the design-lead shows the STATE.md task table.
...
**Instead:** Automatic screenshots only after the **final wave** (build complete). Mid-build screenshots available only on explicit user request via `/modulo:verify`. Wave summaries are text-only with quality scores -- no visual captures.
...
**Instead:** Every wave summary ends with "Awaiting user approval to proceed to Wave {N+1}." The design-lead STOPS and WAITS. No next-wave spawning until explicit user approval. This is a HARD gate -- no configuration option to disable it.
...
**What goes wrong:** The design-lead monitors builder progress in real-time and reports each builder's intermediate state to the user. "Builder for 03-hero is on task 3 of 5. Builder for 04-features just started task 2..."

**Why it's harmful:** Creates a monitoring overhead loop. The design-lead should be managing the wave, not live-narrating builder progress. Intermediate states change rapidly and aren't actionable for the user.

**Instead:** Builders work autonomously. The design-lead only reports when a builder finishes (section-complete one-liner) or fails (failure alert). No intermediate progress narration. Users who want real-time detail can check STATE.md task table (which builders update via SUMMARY.md after completion).
